...
class LanguageModelProcessor:
    def __init__(self):
        ...
        with open('system_prompt_response.txt', 'r') as file:
            self.system_prompt_response = file.read().strip()
    ...
    def get_main_response(self, text):
        prompt_response = ChatPromptTemplate.from_messages([
            SystemMessagePromptTemplate.from_template(self.system_prompt_response),
            MessagesPlaceholder(variable_name="chat_history"),
            HumanMessagePromptTemplate.from_template("{text}")
        ])

        conversation_response = LLMChain(
            llm=self.llm,
        ...
        # Get the main response from the LLM
        response = conversation_response.invoke({"text": text})
        end_time = time.time()

        self.memory.chat_memory.add_ai_message(response['text'])  # Add AI response to memory

        elapsed_time = int((end_time - start_time) * 1000)
        print(f"LLM Response ({elapsed_time}ms): {response['text']}")
        return response['text']
    ...
    async def main(self):
        ...
        while True:
            ...
            if "customer done talking" in done_talking_response.lower():
                ...
                # Get the main response from the LLM
                main_response = self.llm.get_main_response(self.customer_query.strip())
                tts = TextToSpeech()
                tts.speak(main_response)
