...
def format_ssml(text):
    ...
    def replace_email_with_ssml(match):
        email = match.group(0)
        spelled_out = ' '.join(email)
        return f"<say-as interpret-as='characters'>{spelled_out}</say-as>"
    ...
    def speak(self, text):
        ...
        try:
            response = self.polly.synthesize_speech(
                ...
                VoiceId='Joanna'  # You can choose other voices available in AWS Polly
            )
...
class ConversationManager:
    ...
    async def main(self):
        ...
        tts = TextToSpeech()  # Initialize TextToSpeech
        ...
        while True:
            ...
            frames = []
            for _ in range(50):  # Collect 1 second of audio (50 * 20ms = 1000ms)
            ...
            if not speech_timestamps:
                if self.silence_start_time is None:
                    ...
                    if silence_duration > 5:
                        ...
                        tts.speak("Hello, are you still there?")
                        self.silence_start_time = None  # Reset the silence timer
            else:
                self.silence_start_time = None  # Reset the silence timer if speech is detected
