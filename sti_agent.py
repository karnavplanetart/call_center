...
class ConversationManager:
    ...
    def transform_email(self, text):
        # Define patterns and their replacements in a specific order
        patterns = {
            r'\bat the rate\b': '@',
            r'\bthe rate\b': '@',
            r'\btherateb': '@',
        ...
        # Remove any leading/trailing whitespace and handle phrases like "my email id is"
        text = re.sub(r'\bmy email id is\b', '', text, flags=re.IGNORECASE).strip()

        # Define common email domains
        email_domains = ["gmail.com", "yahoo.com", "hotmail.com", "outlook.com"]

        # Remove any text between @ and common email domains
        for domain in email_domains:
            text = re.sub(r'@[^ ]*\b' + re.escape(domain), '@' + domain, text, flags=re.IGNORECASE)
    ...
    async def main(self):
        ...
        session_id = "sw-1-8f7cf768-9802-4e41-8877-bda08b738951-sti"  # Replace with actual session ID

        # Loop indefinitely until "goodbye" is detected
        while True:
            await get_transcript(handle_full_sentence)
            
            # Transform the email in the transcription
            self.transcription_response = self.transform_email(self.transcription_response)
