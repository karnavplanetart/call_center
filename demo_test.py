...
def stream_json_request(url, send_data):
...
class LanguageModelProcessor:
    ...
    async def get_main_response(self, text, session_id, query_id, interrupt_flag):
        ...
        try:
            ...
            def handle_streamed_response(line):
                json_object = json.loads(line.decode('utf-8'))
                ...
                for reply in assistant_replies:
                    ...
                    print(f"LLM Response (Query ID {assistant_message['query_id']}): {assistant_message['content']}")

            # Call the streaming request function
            await asyncio.to_thread(stream_json_request, api_url, payload)
