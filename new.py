...
model, (get_speech_timestamps, save_audio, read_audio, VADIterator, collect_chunks) = torch.hub.load(
    repo_or_dir='snakers4/silero-vad', model='silero_vad', force_reload=True, trust_repo=True
)
...
class VoiceActivityDetector:
    ...
    def is_speech(self, audio_chunk):
        # Convert the audio_chunk to a tensor for the VAD model
        audio_tensor = torch.tensor(np.frombuffer(audio_chunk, dtype=np.int16), dtype=torch.float32)
        audio_tensor = audio_tensor.unsqueeze(0)  # Add batch dimension
        timestamps = self.get_speech_timestamps(audio_tensor, self.model)
...
class ConversationManager:
    def __init__(self):
        ...
        self.vad = VoiceActivityDetector(model, get_speech_timestamps)
        self.sample_rate = 16000
        self.channels = 1
        self.blocksize = 160  # 10ms frames
        self.audio_chunk = None

    def audio_callback(self, indata, frames, time, status):
        if status:
            print(f"Status: {status}")
        self.audio_chunk = indata
    ...
    def get_audio_chunk(self):
        return self.audio_chunk.tobytes() if self.audio_chunk is not None else b''
